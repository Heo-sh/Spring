...
public class VisitJsonController {
	...
	public VisitJsonController(VisitDAO visit_dao) {
		this.visit_dao = visit_dao;
	}
	
	//기존에 했던 visit(방명록)을 list로 담아 json 배열 형태로 변환
	...
	//json형태로 확인을 하려면 insert.do가 아닌 insert_form.do로 가서 확인해야한다.
	@ResponseBody
	//name=???&content=???&pwd=???
	public HashMap<String, String> insert(VisitVO vo, HttpServletRequest request) {
		...
		//res = 1이면 success를 0dlaus fail을 return해보자
